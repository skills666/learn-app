...
C.style.cssText='position:fixed;bottom:-40px;right:-40px;width:340px;height:400px;pointer-events:none;z-index:999;transition:transform .3s ease,opacity .3s ease';
...
function R(){const m=window.matchMedia('(max-width:768px)').matches;W=m?170:340;H=m?200:400;flr=H*.76;const d=Math.min(devicePixelRatio||1,2);C.width=W*d;C.height=H*d;C.style.width=W+'px';C.style.height=H+'px';C.style.right=m?'-20px':'-40px';X.setTransform(d,0,0,d,0,0);}
